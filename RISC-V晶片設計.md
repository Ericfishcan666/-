...
module program_rom (
    ...
    reg [31:0] rom [0:1023]; // 簡易 4KB 模擬
    assign inst = rom[addr[11:2]]; // Word Aligned Address
    
    initial begin
        // 這裡可以載入 hex 檔案: $readmemh("code.hex", rom);
        // 或者手動初始化測試指令
...
module stage_if_id (
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            pc_out <= 0; inst_out <= 32'h00000013; // Reset to NOP
        end else begin
...
module register_file (
    input  wire clk,
    input  wire [4:0] addr_rs1, addr_rs2, addr_rd,
    ...
    assign rs1_value = (addr_rs1==0) ? 0 : gpr[addr_rs1];
    assign rs2_value = (addr_rs2==0) ? 0 : gpr[addr_rs2];

    always @(posedge clk) begin
        if (write_en && addr_rd != 0) gpr[addr_rd] <= write_data;
    end
endmodule
```
...
module forwarding_unit (
    input  wire [4:0] addr_rs1, addr_rs2, addr_rd_ex,
    input  wire reg_write_ex,
    output reg  sel_rs1_forward, sel_rs2_forward
);
    always @(*) begin
        sel_rs1_forward = (reg_write_ex && addr_rd_ex != 0 && addr_rd_ex == addr_rs1);
        sel_rs2_forward = (reg_write_ex && addr_rd_ex != 0 && addr_rd_ex == addr_rs2);
    end
endmodule
```
...
module mul_unit (
    ...
    wire signed [63:0] res_ss = $signed(a) * $signed(b);
    always @(*) begin
        if (funct3 == 0) mul_out = res_ss[31:0]; // MUL (Lower 32 bits)
        else mul_out = res_ss[63:32];            // MULH (Upper 32 bits)
    end
endmodule
```
...
module div_unit (
    ...
    always @(*) begin
        if (b != 0) div_out = $signed(a) / $signed(b);
        else div_out = -1; // Division by zero handling
    end
endmodule
```
...
module lsu (
    ...
    reg [7:0] ram [0:1023]; // 簡易 1KB RAM 供測試
    
    // Store Logic
    always @(posedge clk) begin
        if (write_en) begin 
            // 這裡以 SW (Word) 為例，完整應包含 SB, SH
